...
## [2026-06-27] Sheet-driven invite minting — one-way sync (branch `claude/invite-code-lloyd-hugo-5kbtn5`)

Bulk-minting path so Doug can run beta invites from a single Google Sheet instead of one-off `mint_invite.py` calls. Sheet = source of truth; a sync pushes new rows into `invite_codes`. Spec: [invite_sheet_sync.md](spec/invite_sheet_sync.md). Builds on [invite_codes_per_friend.md](spec/invite_codes_per_friend.md) (schema + redeem unchanged).

- **Decision (with Doug):** **option A — one-way, in-session.** Zero Google Cloud setup. Option B (service account + GitHub Action for two-way status write-back into the sheet) was deferred: the connected Google Drive integration can **read** a sheet but has **no cell-edit tool**, so true write-back needs a service account Doug didn't want for ~10 invitees. Script is written so B is an additive upgrade later.
- **`scripts/sync_invites_from_sheet.py`** — reads the sheet on stdin (or `--csv`), auto-detecting **Markdown pipe-table** (what the Drive integration returns) or **CSV**. Uses 3 columns: `Code` (required, upper-cased), `Email`→`label` (falls back to `Recipient`), `Value` (`"50 credits"`→`credits`, default 50). Batch **insert-only** upsert (`Prefer: resolution=ignore-duplicates`) → idempotent: new codes mint, existing/redeemed codes left untouched, re-runs always safe. `--dry-run` previews with no creds/network.
- **In-session flow:** Doug edits the sheet → says "sync the invite codes" → Claude reads it via Google Drive, pipes to the script, reports `N minted · M existed` (+ optional `list_invites.py`). **One-time setup:** add `SUPABASE_URL`+`SUPABASE_SERVICE_KEY` to the Claude Code web environment (service key is intentionally absent from cloud sessions); local runs read them from the workspace `.env` like the other scripts.
- **Verified (dry-run):** real sheet (GEORGE06/JOSH06/ANGUS06 + a new HUGO06 row) parses all 4 codes, skips the header + `:-:` rule row (fixed a separator-regex bug that required ≥2 dashes); CSV path handles custom credits, recipient fallback, case-normalisation, blank-code skip; non-dry run with no creds fails loudly after parse. Live upsert pends `SUPABASE_*` creds in the run environment.
- **Note:** HUGO06 for Lloyd.hugo@me.com (the original request) is staged in the test input but **not yet live** — it needs a row added to the actual tracker sheet, then a real (credentialled) sync run. Insert-only means changing an existing code's grant in the sheet won't propagate (protects redeemed codes); edit live codes in Supabase directly.
